...
def calc_wcss_for_cluster(i, X, labels, centroids):
    return np.sum((X[labels == i] - centroids[i])**2)

def k_means(k, X, max_iters=100, tol=1e-4):
    ...
    func_parcial = partial(calc_wcss_for_cluster, X=X, labels=labels, centroids=centroids)  
    results = pool.map(func_parcial, range(k))
...
def main():
    ...
    for k in range(1, K_MAX + 1):
        centroids, labels, wcss = k_means(k,X)
