"""Lab 1, part two: parallel k-means over ``proteins.csv`` with multiprocessing.

Same program as the serial version, parallelized following the data-parallel
(SPMD) paradigm: the dataset is split into one chunk per process, every worker
runs the same map step over its own chunk, and the master reduces the partial
results into the new centroids. Processes have separate address spaces, so
they are not limited by the GIL.
...
# Several chunks per process, instead of exactly one: the pool hands them out
# as workers become free, so the fast cores take more chunks than the slow
# ones and no iteration waits for a straggler. Measured on a 16-core machine,
# this is worth about 17% of the total time; anything between 2 and 8 chunks
# per process performs the same, and one chunk each is clearly worse.
...
# Set once per worker by :func:`init_worker`, so that every iteration only has
# to send the bounds of a chunk instead of the data itself.
...
def split(size: int, parts: int) -> Chunks:
    """Split ``range(size)`` into ``parts`` chunks of almost equal length.

    Args:
        size: Number of proteins.
        parts: Number of chunks to split the dataset into.

    Returns:
        The ``(start, stop)`` bounds of every chunk.
    """
...
def init_worker(points: Points) -> None:
    """Store the dataset in the worker, once, when the pool is created."""
    global worker_points  # noqa: PLW0603
    worker_points = points


def chunk_step(start: int, stop: int, centroids: Points) -> Partial:
    """Map step: what one chunk contributes to the next iteration.

    Args:
        start: First protein of the chunk.
        stop: Protein after the last one of the chunk.
        centroids: Current centroids, the same for every worker (SPMD).

    Returns:
        The partial counts, feature sums and inertia of the chunk.
    """
...
def kmeans(
    ...
    Every iteration is a map-reduce round: the workers run :func:`chunk_step`
    over their own chunk and the master combines the partial results. Only the
    centroids travel to the workers and only the partial sums come back.

    Args:
        pool: Pool of worker processes holding the dataset.
        chunks: The ``(start, stop)`` bounds of every chunk.
